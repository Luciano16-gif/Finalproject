...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3004/signup', userData); 
      verify();
    } catch (error) {
        console.error(error);
    }
    ...
    <div>
      ...
      <div className='flex items-center justify-center h-screen w-screen antialiased bg-slate-200'>
        <form className='flex px-3 py-3 flex-col shadow-xl bg-white shadow-indigo-600 rounded-3xl w-1/3 h-50' onSubmit={handleSubmit}>
          <h1 className='self-center text-2xl'>Registrarse</h1>

          <label htmlFor="Nombre">Nombre:</label>
          <input type="text" className="border-solid ps-1 border-black border-2 rounded-md bg-gray-300" name="Nombre" id="Nombre" 
          required maxLength="40" pattern="^[a-zA-Z\s]*$" value={userData.Nombre} onChange={handleChange}/>
          ...
          required maxLength="40" pattern="^[a-zA-Z\s]*$" value={userData.Apellido} onChange={handleChange} />

          <label htmlFor="Usuario" className="mt-2">Nombre de usuario:</label>
          <input type="text" className="border-solid ps-1 border-black border-2 rounded-md bg-gray-300" name="Usuario" id="Usuario" 
          maxLength="40" pattern="^[a-zA-Z0-9\s]*$" value={userData.Usuario} onChange={handleChange}/>
